...
async function fetchPerformanceData(token, portfolioId) {
...
async function fetchRiskData(token, portfolioId) {
...
async function fetchAttributionData(token, portfolioId) {
...
async function fetchConstructionData(token, portfolioId) {
...
async function fetchSpecializedData(token, portfolioId) {
...
app.get('/advanced-analytics', requireAuth, async (req, res) => {
    const token = res.locals.token;
    const selectedTab = req.query.tab || 'performance';
    const selectedPortfolio = req.query.portfolio || 'all';
...
app.get('/holdings', requireAuth, async (req, res) => {
    ...
    res.render('pages/holdings', {
        ...
        fmt: {
            ...
            number: (v) => (v || 0).toLocaleString('en-US')
        }
    });
});
...
app.get('/sectors', requireAuth, async (req, res) => {
    ...
    if (allocationData?.success && allocationData?.data?.allocations) {
        ...
        sectorsData = {
            sectors: allocationData.data.allocations.map((alloc, index) => ({
                ...
                topHoldings: alloc.holdings?.slice(0, 3).map((h) => ({
                    symbol: h.symbol,
...
app.get('/sector-analysis', requireAuth, async (req, res) => {
    const token = res.locals.token;
    const selectedTab = req.query.tab || 'overview';
    const portfolioId = req.query.portfolio || 'all';
    const period = req.query.period || '1M';
    try {
...
app.get('/research-center', requireAuth, async (req, res) => {
    ...
    const activeTab = req.query.tab || 'chart';
    try {
...
app.get('/portfolio-tools', requireAuth, async (req, res) => {
    const token = res.locals.token;
    const selectedPortfolio = req.query.portfolio || 'all';
    const selectedTool = req.query.tool || 'rebalancing';
    // Fetch portfolios list
    const portfoliosData = await apiFetch('/portfolios', token);
    const portfolios = portfoliosData.error ? [] : portfoliosData;
