...
app.use('/api', createProxyMiddleware({
  ...
  pathRewrite: (path, req) => {
    // When mounted at /api, the path received here is already stripped of /api
    // e.g., /auth/login instead of /api/auth/login
    // We need to add /api back for the backend
  ...
  on: {
    proxyReq: (proxyReq, req: any, res) => {
      ...
      // Log what's actually being proxied
      const targetPath = '/api' + (req.url.startsWith('/api') ? req.url.substring(4) : req.url);

      if (token) {
        proxyReq.setHeader('Authorization', `Bearer ${token}`);
        console.log(`[Proxy] ${req.method} ${req.originalUrl} -> ${API_URL}${targetPath} [Token: ${token.substring(0, 20)}...]`);
      } else {
        console.log(`[Proxy] ${req.method} ${req.originalUrl} -> ${API_URL}${targetPath} [NO TOKEN]`);
      }
    },
...
app.get('/add-holding', requireAuth, async (req, res) => {
  const token = res.locals.token;
  const portfolioId = req.query.portfolio_id as string | undefined;

  // Fetch portfolios for the dropdown
  const portfoliosResult = await apiFetch('/portfolios', token);
  const portfolios = portfoliosResult.error ? [] : (Array.isArray(portfoliosResult) ? portfoliosResult : []);

  // Find selected portfolio if ID provided
  let selectedPortfolio = null;
  if (portfolioId) {
    selectedPortfolio = portfolios.find((p: any) => p.id === portfolioId) || null;
  }

  res.render('pages/add-holding', {
    pageTitle: 'Add New Holding',
    portfolios,
    selectedPortfolio,
    token
  });
...
app.get('/analytics', requireAuth, async (req, res) => {
  ...
  const fullHistory = analytics?.fullHistory || [];
  const holdings = analytics?.holdings || [];
  const drawdown = analytics?.drawdown || { series: [], maxDrawdown: 0, currentDrawdown: 0, recoveryDays: 0, maxDrawdownDate: null };
  const returnsDistribution = analytics?.returnsDistribution || { labels: [], data: [], mean: 0, stdDev: 0, normalCurve: [], skewness: 0, kurtosis: 0 };
  const correlation = analytics?.correlation || { symbols: [], matrix: [] };
  const benchmarkList = analytics?.benchmarkList || [];

  // New real data fields
  const rollingReturns = analytics?.rollingReturns || { '30': [], '60': [], '90': [], '252': [] };
  const monthlyReturns = analytics?.monthlyReturns || {};
  const annualReturns = analytics?.annualReturns || [];
  const returns = analytics?.returns || { daily: [], monthly: [], annual: [] };
...
app.get('/revenue-per-employee', requireAuth, async (req, res) => {
  const token = res.locals.token;
  const symbol = (req.query.symbol as string) || 'AAPL';
  const [financials, profile, portfoliosData] = await Promise.all([
    apiFetch(`/analysis/financials/${symbol}`, token),
    apiFetch(`/analysis/profile/${symbol}`, token),
    apiFetch('/portfolios', token)
  ]);
  const portfoliosRaw = portfoliosData.error ? [] : (Array.isArray(portfoliosData) ? portfoliosData : []);
  const safePortfolios = portfoliosRaw.map((p: any) => ({
    id: p.id,
  ...
  res.render('pages/revenue-per-employee', {
    ...
    financials: financials.error ? null : financials,
