...
const taxHarvestingRoutes = require('./routes/taxHarvesting');
...
app.use('/api/news', rssNewsRoutes);
...
// Crypto holdings, market data, transactions, DeFi positions
app.use('/api/crypto', cryptoRoutes);

// ==================== TAX-LOSS HARVESTING ROUTES ====================
// Tax preferences, harvesting opportunities, wash sale tracking
app.use('/api/tax', taxHarvestingRoutes);
...
app.use('/api/efficiency', authenticate, efficiencyRoutes);
...
async function main() {
  try {
    ...
    // Initialize EFFICIENCY DATA SCHEDULER (refresh at 6 PM EST daily)
    const efficiencyScheduler = require('./services/efficiencyScheduler');
    efficiencyScheduler.start();
    logger.debug('✓ Efficiency data scheduler initialized (6 PM EST daily)');
