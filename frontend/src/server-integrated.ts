...
// API fetch helper
async function apiFetch(endpoint: string, token: string | null = null, options: any = {}) {
...
app.get('/', requireAuth, async (req, res) => {
  ...
  const [
    ...
    apiFetch('/transactions?limit=10', token),
    apiFetch('/watchlist', token),
    apiFetch('/alerts?limit=5', token),
    apiFetch('/market/indices', token)
  ]);

  const portfolios = portfoliosData.error ? [] : portfoliosData;
  const dashboard = dashboardData.error ? null : dashboardData;
  const holdings = holdingsData.error ? [] : (Array.isArray(holdingsData) ? holdingsData : []);
  const transactions = transactionsData.error ? [] : (Array.isArray(transactionsData) ? transactionsData : (transactionsData.transactions || []));
  const watchlist = watchlistData.error ? [] : (Array.isArray(watchlistData) ? watchlistData : []);
  const alerts = alertsData.error ? [] : (Array.isArray(alertsData) ? alertsData : (alertsData.alerts || []));
  const indices = marketData.error ? [] : (Array.isArray(marketData) ? marketData : (marketData.indices || []));

  // Calculate totals from holdings
  const calculatedTotals = holdings.reduce((acc: any, h: any) => ({
    value: acc.value + ((h.shares || 0) * (h.currentPrice || h.price || 0)),
    cost: acc.cost + ((h.shares || 0) * (h.avgCost || h.avg_cost_basis || 0)),
    gain: acc.gain + ((h.shares || 0) * ((h.currentPrice || h.price || 0) - (h.avgCost || h.avg_cost_basis || 0))),
    dayChange: acc.dayChange + ((h.shares || 0) * (h.dayChange || 0)),
    income: acc.income + (h.annualDividend || 0),
    holdingsCount: acc.holdingsCount + 1
  }), { value: 0, cost: 0, gain: 0, dayChange: 0, income: 0, holdingsCount: 0, cash: 0, ytdReturn: 0 });

  // Use calculated totals if dashboard data is missing
  const totals = dashboard ? {
    value: dashboard.totalValue || calculatedTotals.value,
    cost: dashboard.totalCost || calculatedTotals.cost,
    gain: dashboard.totalGain || calculatedTotals.gain,
    dayChange: dashboard.dayChange || calculatedTotals.dayChange,
    income: dashboard.annualDividends || calculatedTotals.income,
    holdingsCount: dashboard.holdingsCount || calculatedTotals.holdingsCount,
    cash: dashboard.cash || 0,
    ytdReturn: dashboard.ytdReturn || (calculatedTotals.cost > 0 ? (calculatedTotals.gain / calculatedTotals.cost) * 100 : 0)
  } : calculatedTotals;
  ...
  const sectors = Array.from(sectorMap.entries())
    .map(([sector, value]) => ({
      ...
      sector: sector,
      value: value,
      weight: totals.value > 0 ? (value as number / totals.value) * 100 : 0
    }))
    .filter(s => s.value > 0);

  // Risk metrics
  const risk = dashboard?.riskMetrics ? {
    beta: dashboard.riskMetrics.beta || 1.0,
    sharpe: dashboard.riskMetrics.sharpeRatio || 0,
    volatility: dashboard.riskMetrics.volatility || 0,
    maxDrawdown: dashboard.riskMetrics.maxDrawdown || 0
  } : { beta: 1.0, sharpe: 0, volatility: 0, maxDrawdown: 0 };
...
app.get('/holdings', requireAuth, async (req, res) => {
  ...
  if (!portfolios.error) {
    for (const p of portfolios) {
      if (p.holdings) {
        allHoldings.push(...p.holdings.map((h: any) => ({ ...h, portfolioName: p.name })));
      }
    }
  }
...
app.get('/transactions', requireAuth, async (req, res) => {
  ...
  if (portfolioId) params.append('portfolioId', portfolioId as string);
