...
router.get('/', [
  ...
  try {
    ...
    const { portfolio_id, symbol, type, startDate, endDate } = req.query;
...
router.get('/all', async (req, res) => {
  try {
    ...
    const { portfolio_id, symbol, type, limit = 100 } = req.query;
...
router.post('/', [
  ...
  try {
    ...
    const { portfolio_id, symbol, type, executedAt, notes } = req.body;
    ...
    if (type === 'buy') {
      ...
      try {
        await prisma.portfolios.update({
          ...
          data: { cash_balance: { increment: amount } }
        });
      } catch (err) {
...
router.post('/import', [
  ...
  try {
    const { portfolio_id, transactions } = req.body;
