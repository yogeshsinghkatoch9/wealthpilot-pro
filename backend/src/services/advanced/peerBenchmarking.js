...
class PeerBenchmarkingService {
  async compareToPeers(portfolioId) {
    const portfolio = await prisma.portfolios.findUnique({ where: { id: portfolioId } });
    if (!portfolio) return { percentileRank: 0, peerComparison: [] };

    const allPortfolios = await prisma.portfolios.findMany({
      where: { user_id: portfolio.userId },
