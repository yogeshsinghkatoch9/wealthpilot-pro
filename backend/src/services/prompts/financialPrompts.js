...
5. Recommended positioning adjustments`
   ...
   * Chat context prompt
   */
  chatSystemPrompt: (portfolioContext) => {
    let contextStr = '';

    // Include regular portfolio context
    if (portfolioContext && portfolioContext.holdings?.length > 0) {
      contextStr += `USER'S PORTFOLIO:
- Total Value: $${portfolioContext.totalValue?.toLocaleString() || 'N/A'}
- Number of Holdings: ${portfolioContext.holdingCount || portfolioContext.holdings?.length || 0}
- Holdings: ${portfolioContext.holdings?.slice(0, 15).map(h =>
  `${h.symbol || h.name} (${h.shares || h.quantity} shares, $${(h.marketValue || h.value || 0).toLocaleString()})`
).join(', ')}
`;
    }

    // Include uploaded portfolio data
    if (portfolioContext?.uploadedPortfolio) {
      const up = portfolioContext.uploadedPortfolio;
      contextStr += `
UPLOADED PORTFOLIO FILE:
- Total Holdings: ${up.summary?.totalHoldings || up.holdings?.length || 0}
- Total Value: $${up.summary?.totalValue?.toLocaleString() || 'N/A'}
- Total Gain/Loss: $${up.summary?.totalGL?.toLocaleString() || 'N/A'} (${up.summary?.totalGLPercent?.toFixed(2) || 0}%)
- Est. Annual Income: $${up.summary?.totalEstAnnualIncome?.toLocaleString() || 'N/A'}
- Portfolio Yield: ${up.summary?.portfolioYield?.toFixed(2) || 0}%

DETAILED HOLDINGS:
${up.holdings?.slice(0, 25).map(h => {
  let details = `- ${h.symbol || h.description}: `;
  if (h.quantity) details += `${h.quantity} shares, `;
  if (h.value) details += `Value: $${h.value.toLocaleString()}, `;
  if (h.price) details += `Price: $${h.price.toFixed(2)}, `;
  if (h.oneDayPriceChange !== undefined) details += `1D Change: ${h.oneDayPriceChange >= 0 ? '+' : ''}${h.oneDayPriceChange.toFixed(2)}%, `;
  if (h.principalGL) details += `G/L: $${h.principalGL.toLocaleString()} (${h.principalGLPercent?.toFixed(2) || 0}%), `;
  if (h.currentYield) details += `Yield: ${h.currentYield.toFixed(2)}%, `;
  if (h.assetCategory || h.assetType) details += `Type: ${h.assetCategory || h.assetType}, `;
  if (h.accountType) details += `Account: ${h.accountType}, `;
  if (h.purchaseDate) details += `Purchased: ${h.purchaseDate}`;
  return details.replace(/, $/, '');
}).join('\n') || 'No holdings data'}
`;
    }

    return `You are WealthPilot AI, an advanced financial assistant.

${contextStr}

Your capabilities:
1. Answer questions about investments and portfolio management
2. Provide detailed analysis of portfolio holdings, sectors, and performance
3. Calculate and explain financial metrics
4. Identify risk exposures and diversification issues
5. Provide market analysis and insights
6. Explain financial concepts and metrics
7. Offer general investment guidance (with appropriate disclaimers)
8. Help users understand their portfolio performance

When analyzing uploaded portfolio data:
- Provide comprehensive breakdown by asset type, sector, and account
- Calculate 1-day value changes based on the 1-day price change percentages
- Analyze yield and income characteristics
...
- Format responses with clear headers, bullet points, and tables when appropriate
