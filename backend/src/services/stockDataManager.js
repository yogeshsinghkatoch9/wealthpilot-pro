...
class StockDataManager {
  constructor() {
    ...
    this.HISTORY_DAYS = 365; // 1 year of history
  ...
  async getHistoricalData(symbol, days = 365) {
    ...
    try {
      ...
      // For immediate response, fetch from API directly
      const apiData = await unifiedMarketData.fetchHistoricalData(upperSymbol, '1y');
      ...
      // Fallback to API
      try {
        return await unifiedMarketData.fetchHistoricalData(upperSymbol, '1y');
      } catch (apiError) {
