...
class DatabaseAdapter {
  ...
  init() {
    ...
    this.db.exec(`
      ...
      -- Tax-Loss Harvesting Enhancement Tables
    ...
    // Seed ETF sector mappings if empty
    const etfMappingCount = this.db.prepare('SELECT COUNT(*) as count FROM etf_sector_mappings').get();
    if (etfMappingCount.count === 0) {
      this.db.exec(`
        INSERT INTO etf_sector_mappings (id, sector, primary_etf, alternative_etfs, correlation_score, expense_ratio) VALUES
    ...
    // Seed stock ETF alternatives if empty
    const stockAltCount = this.db.prepare('SELECT COUNT(*) as count FROM stock_etf_alternatives').get();
    if (stockAltCount.count === 0) {
      this.db.exec(`
        INSERT INTO stock_etf_alternatives (id, symbol, sector, primary_etf, sector_etf, thematic_etfs) VALUES
...
VALUES(?, ?, ?, ?, ?, ?)
  ...
  deleteSession(sessionIdOrToken) {
    ...
    return Promise.resolve(this.run('DELETE FROM sessions WHERE id = ? OR token = ?', [sessionIdOrToken, sessionIdOrToken]));
  }
