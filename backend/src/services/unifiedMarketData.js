...
// Cryptocurrency symbol mapping (ticker -> CoinGecko ID)
...
class UnifiedMarketDataService {
  constructor() {
    ...
    this.coincapKey = process.env.COINCAP_API_KEY;
    ...
    logger.info('Unified Market Data Service initialized with 6 providers (including Yahoo Finance + CoinCap)');
  }
  ...
  async fetchQuote(symbol) {
    ...
    return this.getCached(cacheKey, this.cacheTTL.quote, async () => {
      ...
      if (isCryptocurrency(symbol)) {
        try {
          const quote = await this.fetchQuoteCoinCap(symbol);
          if (quote) {
            logger.info(`[Quote] SUCCESS via CoinCap: ${symbol}`);
            return quote;
          }
        } catch (err) {
          logger.warn(`[Quote] CoinCap failed for ${symbol}:`, err.message);
        }
      }

      // Try Yahoo Finance FIRST for stocks (PRIMARY - no API key required, most reliable)
      try {
  ...
  /**
   * CoinGecko Quote for Cryptocurrencies (free, no API key needed)
   */
  async fetchQuoteCoinCap(symbol) {
    ...
    try {
      // Use CoinGecko API (more reliable, no API key needed)
      const url = `https://api.coingecko.com/api/v3/simple/price?ids=${cryptoId}&vs_currencies=usd&include_24hr_change=true&include_24hr_vol=true&include_market_cap=true`;
      const response = await axios.get(url, {
      ...
      // Map CoinGecko ID back to friendly name
      const cryptoNames = {
        'bitcoin': 'Bitcoin',
        'ethereum': 'Ethereum',
        'solana': 'Solana',
      ...
      logger.warn(`[Quote] CoinGecko error for ${symbol}:`, err.message);
      return null;
    }
  }
