...
router.get('/preferences', async (req, res) => {
  try {
    ...
    const preferences = await taxLossHarvesting.getUserTaxPreferences(userId);
...
router.put('/preferences', async (req, res) => {
  try {
    ...
    const updated = await taxLossHarvesting.updateUserTaxPreferences(userId, preferences);
...
router.get('/wash-sales/:portfolioId', async (req, res) => {
  try {
    ...
    const washSales = await taxLossHarvesting.getWashSaleWindows(userId, portfolioId);
...
router.post('/harvest/preview', async (req, res) => {
  try {
    const userId = req.user.id;
    const { portfolio_id, symbol, replacementSymbol } = req.body;
...
router.post('/harvest/execute', async (req, res) => {
  try {
    const userId = req.user.id;
    const { portfolio_id, symbol, replacementSymbol, lotMethod } = req.body;
    ...
    if (result.success) {
      await taxLossHarvesting.recordHarvestHistory(userId, portfolioId, {
        symbol,
...
router.get('/history/:portfolioId', async (req, res) => {
  try {
    ...
    const history = await taxLossHarvesting.getHarvestHistory(
      userId,
      portfolioId,
      parseInt(limit)
    );
...
router.get('/carryforward', async (req, res) => {
  try {
    ...
    const carryforward = await taxLossHarvesting.getCarryforwardBalance(userId);
...
router.post('/carryforward', async (req, res) => {
  try {
    ...
    const result = await taxLossHarvesting.updateCarryforward(userId, taxYear, {
...
router.get('/recommendation/:portfolioId/:symbol', async (req, res) => {
  try {
    const { portfolio_id, symbol } = req.params;
    const userId = req.user.id;

    // Get wash sale windows to exclude
    const washSaleWindows = await taxLossHarvesting.getWashSaleWindows(userId, portfolioId);
