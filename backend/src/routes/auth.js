...
router.post('/register', [
  ...
  try {
    ...
    await prisma.watchlists.create({
      ...
      logger.warn('Failed to create default watchlist:', err.message);
    });
  ...
  try {
    ...
    if (user.two_factor_enabled) {
      ...
      const tempToken = jwt.sign(
        { user_id: user.id, email: user.email, pending2FA: true },
        JWT_SECRET,
    ...
    const token = jwt.sign(
      { user_id: user.id, email: user.email, sessionId },
      JWT_SECRET,
...
router.post('/refresh', authenticate, async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { user_id: req.user.id, email: req.user.email, sessionId },
      JWT_SECRET,
      { expiresIn: process.env.JWT_EXPIRES_IN || '7d' }
    );
