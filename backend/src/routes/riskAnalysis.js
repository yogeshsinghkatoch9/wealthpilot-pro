...
router.post('/stress-test/custom', async (req, res) => {
  try {
    const { portfolio_id, scenarios } = req.body;
...
router.get('/correlation', async (req, res) => {
  try {
    const { portfolio_id, symbols } = req.query;
    ...
    res.json({
      success: true,
      portfolio_id: portfolioId || 'custom',
      ...correlationMatrix
    });
  } catch (error) {
...
router.get('/var', async (req, res) => {
  try {
    const { portfolio_id, confidence = 95, horizon = 1 } = req.query;
    const holdings = await getPortfolioHoldings(req.user.id, portfolioId);
