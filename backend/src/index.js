...
// Crypto portfolio routes with error handling
let cryptoRoutes;
try {
  cryptoRoutes = require('./routes/crypto');
  console.log('[INDEX] Crypto routes loaded successfully');
} catch (error) {
  console.error('[INDEX] Failed to load crypto routes:', error.message);
  const expressRouter = require('express').Router;
  cryptoRoutes = expressRouter();
  cryptoRoutes.all('*', (req, res) => {
    res.status(500).json({ success: false, error: 'Crypto routes failed to load' });
  });
}
...
app.use('/api/crypto', cryptoRoutes); // Crypto portfolio
